...
package body Attr_Float is
   ...
   Float_Radix        : constant Float := Float (Float'Machine_Radix);
   Radix_To_M_Minus_1 : constant Float := Float_Radix ** (Float'Machine_Mantissa - 1);
   ...
   Rad    : constant Float := Float (Float'Machine_Radix);
   Invrad : constant Float := 1.0 / Rad;
   ...
   R_Power : constant array (Expbits) of Float :=
     (Rad **  1,
      Rad **  2,
      Rad **  4,
      Rad **  8,
   ...
   R_Neg_Power : constant array (Expbits) of Float :=
     (Invrad **  1,
      Invrad **  2,
      Invrad **  4,
      Invrad **  8,
   ...
   procedure Decompose (XX : Float; Frac : out Float; Expo : out UI);
   ...
   function Gradual_Scaling (Adjustment : UI) return Float;
   ...
   function Ceiling (X : Float) return Float is
      XT : constant Float := Truncation (X);
   begin
      if X <= 0.0 then
         return XT;
      elsif X = XT then
   ...
   function Compose (Fraction : Float; Exponent : UI) return Float is
      Arg_Frac : Float;
      Arg_Exp  : UI;
   ...
   function Copy_Sign (Value, Sign : Float) return Float is
      ...
      function Is_Negative (V : Float) return Boolean;
   ...
   procedure Decompose (XX : Float; Frac : out Float; Expo : out UI) is
      X : constant Float := Float'Machine (XX);
   ...
   begin
      ...
      elsif X > Float'Safe_Last then
         Frac := Invrad;
         Expo := Float'Machine_Emax + 1;

      elsif X < Float'Safe_First then
         Frac := -Invrad;
         Expo := Float'Machine_Emax + 2;    -- how many extra negative values?
   ...
   function Exponent (X : Float) return UI is
      X_Frac : Float;
      X_Exp  : UI;
   ...
   function Gradual_Scaling  (Adjustment : UI) return Float is
      Y  : Float;
      Y1 : Float;
      Ex : UI := Adjustment;

   begin
      if Adjustment < Float'Machine_Emin - 1 then
         Y  := 2.0 ** Float'Machine_Emin;
         Y1 := Y;
         Ex := Ex - Float'Machine_Emin;
         while Ex < 0 loop
            Y := Float'Machine (Y / 2.0);
   ...
   function Leading_Part (X : Float; Radix_Digits : UI) return Float is
      L    : UI;
      Y, Z : Float;

   begin
      if Radix_Digits >= Float'Machine_Mantissa then
         return X;
   ...
   begin
      ...
      --  i.e. decreasing the exponent by that amount.
         ...
         if X_Frac = 0.5 then
            return X - Gradual_Scaling (X_Exp - Float'Machine_Mantissa - 1);

         --  Otherwise the exponent is unchanged

         else
            return X - Gradual_Scaling (X_Exp - Float'Machine_Mantissa);
         end if;
      end if;
   end Pred;
   ...
   function Scaling (X : Float; Adjustment : UI) return Float is
   begin
   ...
   begin
      ...
      if X = 0.0 then
         X1 := 2.0 ** Float'Machine_Emin;

         --  Following loop generates smallest denormal

         loop
            X2 := Float'Machine (X1 / 2.0);
            exit when X2 = 0.0;
            X1 := X2;
         end loop;
   ...
   begin
      ...
      if Result >= Radix_To_M_Minus_1 then
         ...
         Result :=
           Float'Machine (Radix_To_M_Minus_1 + Result) - Radix_To_M_Minus_1;
   ...
   function Valid (X : not null access Float) return Boolean is
      IEEE_Emin : constant Integer := Float'Machine_Emin - 1;
      IEEE_Emax : constant Integer := Float'Machine_Emax - 1;
      ...
      --    - Float'Size is an integral multiple of Float_Word'Size
      ...
      Rep_Words : constant Positive :=
                    (Float'Size + Float_Word'Size - 1) / Float_Word'Size;
      Rep_Last  : constant Rep_Index :=
                    Rep_Index'Min
                      (Rep_Index (Rep_Words - 1),
                       (Float'Mantissa + 16) / Float_Word'Size);
      ...
      --  won't be larger than Float'Mantissa + 16.
      ...
      function To_Float is new Ada.Unchecked_Conversion (Float_Rep, Float);
      ...
      E  : constant IEEE_Exponent_Range :=
             Integer ((R (Most_Significant_Word) and Exponent_Mask) /
                                                        Exponent_Factor)
               - IEEE_Bias;
      --  Mask/Shift Float to only get bits from the exponent. Then convert biased
